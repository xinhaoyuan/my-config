...
local function get_offset(screen, position, idx, width, height)
    ...
    elseif position:match("hcenter") then
        v.x = ws.x + math.floor((ws.width - (width + naughty.config.padding)) / 2)
    else
    ...
    elseif position:match("vcenter") then
        v.y = ws.y + math.floor((ws.height - (naughty.config.padding + height)) / 2) - existing
    else
