...
local function eval_exhaustively(maybe_picker, context, data, resolver)
    ...
    while true do
        ...
        if is_picker(maybe_picker) then
            ...
            maybe_picker = maybe_picker(context, data)
        else
            break
        end
    end
...
local function eval_string(components, context, data)
    local ret = ""
    for _, component in ipairs(components) do
        component = eval_exhaustively(component, context, data)
...
function module.constructors.concat(...)
    local components = {...}
    return function (context, data)
        return eval_string(components, context, data)
    end
end
...
function module.constructors.beautiful(...)
    local components = {...}
    return function (context, data)
        local ret = eval_string(components, context, data)
        return ret and beautiful[ret]
    end
end

function module.constructors.context(...)
    local components = {...}
    return function (context, data)
        local ret = eval_string(components, context, data)
        return ret and context[ret]
    end
end
...
function module.constructors.branch(cond, true_branch, false_branch)
    return function (context, data)
        local value = eval_exhaustively(
            cond, context, data, function (value)
        ...
        return function (context, data)
            for i = 1, #args, 2 do
                local cond = eval_exhaustively(
                    args[i], context, data, function (value)
                        if type(value) == "string" then
                            return context[value]
                        end
                        return value
    ...
    constructor = function (...)
        ...
        return function (context, data)
            for _, option in ipairs(options) do
                local ret = eval_exhaustively(option, context, data)
...
module.constructors.list = {
    constructor = function (...)
        ...
        return function (context, data)
            local ret = {}
            for i = 1, #args do
                ret[i] = eval_exhaustively(args[i], context, data)
            end
            return ret
        end
    end,
...
module.constructors.table = {
    ...
    constructor = function (...)
        ...
        return function (context, data)
            local ret = {}
            for i = 1, #args, 2 do
                local key = eval_exhaustively(args[i], context, data)
                local value = eval_exhaustively(args[i + 1], context, data)
                ret[key] = value
            end
            return ret
        end
...
function module.constructors.wrap(f, ...)
    ...
    return function (context, data)
        ...
        for i, arg in ipairs(args) do
            args_eval[i] = eval_exhaustively(arg, context, data)
        end
