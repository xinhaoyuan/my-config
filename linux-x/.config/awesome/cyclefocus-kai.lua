...
cyclefocus.cycle = function(startdirection, _args)
    ...
    focus.ignore_focus_signal = true
    ...
    local get_next_client = function(direction, idx, stack)
        local startidx = idx
        local stack = stack or focus.get_stack()
    ...
    capi.keygrabber.run(function(mod, key, event)
        ...
        local exit_grabber = function (c)
            ...
            focus.ignore_focus_signal = false
            return true
        end
        ...
        local prevnextlist = awful.util.table.clone(focus.get_stack())  -- Use a copy, entries will get nil'ed.
        local _idx = idx
        ...
        for _,i in ipairs(offsets) do
            _idx = dlist[i]
            do_notification_for_idx_offset(i, focus.get_stack()[_idx], _idx, dlist)
